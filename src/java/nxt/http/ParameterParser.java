...
final class ParameterParser {
	...
	public static byte[] getMachineCode(HttpServletRequest req) throws ParameterException{
		try {
			return Convert.parseHexString(req.getParameter("machineCode"));
		} catch (RuntimeException e) {
			throw new ParameterException(INCORRECT_PROGRAMCODE);
		}
	}
    ...
    static int getWaitForNumberOfBlocks(HttpServletRequest req) throws ParameterException {
    	int waitForNumberOfBlocks;
        try {
            waitForNumberOfBlocks = Integer.parseInt(req.getParameter("waitForNumberOfBlocks"));
            if (waitForNumberOfBlocks < 0 || waitForNumberOfBlocks > Constants.MAX_AUTOMATED_TRANSACTIONS_WAITFORNUMBEROFBLOCKS) {
                throw new ParameterException(INCORRECT_QUANTITY);
            }
            return waitForNumberOfBlocks;
        } catch (NumberFormatException e) {
            throw new ParameterException(INCORRECT_QUANTITY);
        }
    }
    
    static long getMinimumFee(HttpServletRequest req) throws ParameterException {
    	Long minimumFee = Convert.parseUnsignedLong(req.getParameter("minimumFee"));
        try {
            if (minimumFee < 0 || minimumFee < Constants.MAX_AUTOMATED_TRANSACTIONS_MINIMUM_FEE_NQT) {
                throw new ParameterException(INCORRECT_QUANTITY);
            }
            return minimumFee;
        } catch (NumberFormatException e) {
	...
	public static byte[] getVariables( HttpServletRequest req ) throws ParameterException {
		try {
			return Convert.parseHexString(req.getParameter("variables"));
		} catch (RuntimeException e) {
