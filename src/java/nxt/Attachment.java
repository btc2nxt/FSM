...
*/
    ...
    /*public final static class AutomatedTransactionNxtPayment extends AbstractAttachment{

    	private final Long atId;
    	private final Long quantityNQT;
    	private final Long minFee;
    	
    	AutomatedTransactionNxtPayment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException{
    		super(buffer,transactionVersion);
    		this.atId = buffer.getLong();
    		this.quantityNQT = buffer.getLong();
    		this.minFee = buffer.getLong();
    		
    		//TODO check if minFee is valid
    	}
    	
    	AutomatedTransactionNxtPayment(JSONObject attachmentData) throws NxtException.NotValidException {
    		super(attachmentData);
    		this.atId = Convert.parseUnsignedLong((String)attachmentData.get("atId"));
    		this.quantityNQT = Convert.parseLong((String)attachmentData.get("quantityNQT"));
    		this.minFee = Convert.parseLong((String)attachmentData.get("minFee"));
    	}
    	
    	AutomatedTransactionNxtPayment(Long atId, Long quantityNQT, Long minFee) throws NxtException.NotValidException{
    		this.atId = atId;
		...
		@Override
		public TransactionType getTransactionType() {
			return TransactionType.AutomatedTransactions.AUTOMATED_TRANSACTION_NXT_PAYMENT;
		}

		@Override
		String getAppendixName() {
			return "AutomatedTransactionNxtPayment";
		}
		...
		void putMyBytes(ByteBuffer buffer) {
			buffer.putLong(atId);
			buffer.putLong(quantityNQT);
			buffer.putLong(minFee);
		}

		@Override
		void putMyJSON(JSONObject json) {
			json.put("atId", Convert.toUnsignedLong(atId));
			json.put("quantityNQT", quantityNQT);
			json.put("minFee",minFee);
			
		}
		
		public Long getAtId(){return atId;}
		
		public Long getQuantityNQT() { return quantityNQT;}
		
		public Long getMinFee() { return minFee; }
    	
    }*/

    /*public final static class AutomatedTransactionAssetPayment extends AbstractAttachment{
    	...
    	AutomatedTransactionAssetPayment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException{
    		super(buffer,transactionVersion);
    		this.atId = buffer.getLong();
    		this.assetId = buffer.getLong();
    	...
    	AutomatedTransactionAssetPayment(JSONObject attachmentData) throws NxtException.NotValidException {
    		super(attachmentData);
    		this.atId = Convert.parseUnsignedLong((String)attachmentData.get("atId"));
    		this.assetId = Convert.parseUnsignedLong((String)attachmentData.get("assetId"));
    		this.quantity = Convert.parseLong((String)attachmentData.get("quantity"));
    		this.minFee = Convert.parseLong((String)attachmentData.get("minFee"));
    	}
    	
    	AutomatedTransactionAssetPayment(Long atId, Long assetId, Long quantity, Long minFee) throws NxtException.NotValidException{
    		this.atId = atId;
    		this.assetId = assetId;
    		this.quantity = quantity;
    		this.minFee = minFee;
		...
		@Override
		public TransactionType getTransactionType() {
			return TransactionType.AutomatedTransactions.AUTOMATED_TRANSACTION_ASSET_PAYMENT;
		}

		@Override
		String getAppendixName() {
			return "AutomatedTransactionAssetPayment";
		}
		...
		void putMyJSON(JSONObject json) {
			json.put("atId", Convert.toUnsignedLong(atId));
			json.put("assetId", Convert.toUnsignedLong(assetId));
