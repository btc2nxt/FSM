...
*/
    ...
    public final static class AutomatedTransactionsState extends AbstractAttachment{
		...
		AutomatedTransactionsState(ByteBuffer buffer,
			...
			for ( byte i=0; i < atPaymentsCount ; i++) {
				AT_Transaction tx = new AT_Transaction( AT_API_Helper.getByteArray(buffer.getLong()), buffer.getLong(), null, buffer.getInt(), buffer.getInt(), buffer.getLong());
		...
		AutomatedTransactionsState(JSONObject attachmentData) throws NxtException.NotValidException {
            ...
            if (atPaymentArray.size() >0) {
                for (Object o : atPaymentArray) {
                	...
                	AT_Transaction tx = new AT_Transaction(AT_API_Helper.getByteArray(Convert.parseLong(atPayment.get("recipientId"))), Convert.parseLong(atPayment.get("amount")), null, ((Long)atPayment.get("x")).intValue(), ((Long)atPayment.get("y")).intValue(), Convert.parseLong(atPayment.get("assetId")));
    				this.atPayments.add(tx);
                }
		...
		void putMyJSON(JSONObject attachment) {       
    		...
    		if ( atPayments != null){
    			for (AT_Transaction tx : atPayments )
    			{
                	...
                	json.put("x", tx.getX());
                	json.put("y", tx.getY());
                	json.put("assetId", tx.getAssetId());
    ...
    public final static class GameBeWorker extends GameMove {
    	...
    	GameBeWorker(JSONObject attachmentData) {
      	...
      	public GameBeWorker(short xCoordinate, short yCoordinate, long amountNQT) throws NxtException.NotValidException {
    ...
    public final static class GameBeCollector extends GameMove {
    	...
    	GameBeCollector(JSONObject attachmentData) {
      	...
      	public GameBeCollector(short xCoordinate, short yCoordinate, long amountNQT) throws NxtException.NotValidException {
            super(xCoordinate, yCoordinate);
            this.amountNQT = amountNQT;
        }
